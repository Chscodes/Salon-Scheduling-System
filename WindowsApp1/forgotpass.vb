...
Public Class forgotpass
    ...
    Private Sub TextBox1_TextChanged_1(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        connection.Close()
        connection.Open()
        command = New MySqlCommand("Select * from employee where emp_username = '" & TextBox1.Text & "'", connection)
        dataReader = command.ExecuteReader

        While dataReader.Read
            Dim LName = dataReader.GetString("emp_Lname")
            Dim Fname = dataReader.GetString("emp_Fname")
            Dim add = dataReader.GetString("emp_Add")
            Dim email = dataReader.GetString("emp_email")
